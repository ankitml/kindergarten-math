...
def generate_problems(starting_y_position: float, canvas: Canvas) -> None:
    ...
    for i in range(1, problems_per_column + 1):
        y_position = get_random_problem(x_position, y_position, canvas)
...
def get_random_problem(x: float, y: float, canvas: Canvas) -> float:
    PROBLEM_GENERATORS = [
...
# def generate_single_problem_butterfly(x_position: float, y_position: float, canvas: Canvas) -> float:
